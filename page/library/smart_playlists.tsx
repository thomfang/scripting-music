...
export function RecentlyAddedView() {
  ...
  const [showPlaylistPicker, setShowPlaylistPicker] = useState(false)
  const [selectedMusic, setSelectedMusic] = useState<Music | null>(null)
  const [sortType, setSortType] = useState<SmartSortType>(Storage.get<SmartSortType>("recently_added_sort") ?? "recent")
  const [searchText, setSearchText] = useState("")
  ...
  async function loadMusics() {
    try {
      // getAllMusic 已按 added_at DESC，取前 50 首（只有真正入库的歌，未下载完的已不再预入库）。
      const all = await database.getAllMusic()
  ...
  useEffect(() => {
    setMusics(prev => sortMusics(prev, sortType))
    Storage.set("recently_added_sort", sortType)
  }, [sortType])
  ...
  if (loading) return <LoadingState message="加载最近添加中..." />
  if (musics.length === 0) return <EmptyState icon="clock.badge.plus" title="暂无歌曲" message="下载或添加歌曲后会显示在这里" />

  return (
    <List
      navigationTitle="最近添加"
      searchable={{ value: searchText, onChanged: setSearchText }}
      sheet={{
        isPresented: showPlaylistPicker,
        onChanged: (v: boolean) => { if (!v) { setShowPlaylistPicker(false); setSelectedMusic(null) } },
      ...
      toolbar={
        <Toolbar>
          <ToolbarItem placement="topBarTrailing">
            <Menu label={<Image systemName="arrow.up.arrow.down" />}>
              <Button title="按添加时间" systemImage={sortType === "recent" ? "checkmark" : undefined} action={() => setSortType("recent")} />
              <Button title="按歌曲名称" systemImage={sortType === "title" ? "checkmark" : undefined} action={() => setSortType("title")} />
              <Button title="按艺人名称" systemImage={sortType === "artist" ? "checkmark" : undefined} action={() => setSortType("artist")} />
            </Menu>
          </ToolbarItem>
      ...
      <Section>
        {filtered.map(music => (
          <SongRow
            ...
            queue={filtered}
            coverExists={coverExists}
            placeholderIcon="clock.badge.plus"
            placeholderTint="secondaryLabel"
            onToggleFavorite={toggleFavorite}
            onDelete={deleteMusic}
            onAddToPlaylist={m => { setSelectedMusic(m); setShowPlaylistPicker(true) }}
